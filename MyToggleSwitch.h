...
public:
    ...
    {
        (void)shouldDrawButtonAsHighlighted;   //Do nothing
        (void)shouldDrawButtonAsDown;   //Do nothing
        ...
        auto b = getSwitchBounds().toFloat();
        auto textBound = b.expanded(b.getWidth(), b.getHeight()*1.5f);

        g.setColour(juce::Colour(0xFFC3C3C3));
        g.setFont(16);
        g.drawText(title, textBound, juce::Justification::centredTop, 1);

        auto buttomRow = textBound.reduced(textBound.getWidth() * 0.3f, 0);
...
private:
    ...
    juce::Rectangle<float> getSwitchBounds()
    {
        ...
        auto b = bounds.reduced(bounds.getWidth() * 0.4f, bounds.getHeight() * 0.45f);
